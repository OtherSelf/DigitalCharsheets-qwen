...
const StatBox = ({ label, value, editing, onChange, isCompactView, notes, onNoteChange, hideNotes }: { label: string; value: number; editing: boolean; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; isCompactView: boolean; notes?: string; onNoteChange: (val: string) => void; hideNotes: boolean; }) => {
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center rounded-lg bg-background text-center border relative", isCompactView ? "p-1" : "p-4")}>
      {!hideNotes && (
        <Popover>
          <PopoverTrigger asChild><Button variant={notes ? 'secondary' : 'ghost'} size="icon" className="h-5 w-5 absolute top-1 right-1"><Info className="h-3 w-3" /></Button></PopoverTrigger>
          <PopoverContent className="w-64"><Label className="text-xs mb-2 block">Notes for {label}</Label><Textarea defaultValue={notes || ''} onBlur={(e) => onNoteChange(e.target.value)} placeholder="Add notes..." className="min-h-[100px] text-sm" /></PopoverContent>
        </Popover>
      )}
...
export const DndSheet = React.forwardRef<any, DndSheetProps>(
  ({ character, isCompactView, activeCompactSection }, ref) => {
    ...
    const handleHpMath = (op: 'sub' | 'rec') => {
      ...
      setCombatStats({...combatStats, hitPoints: n}); setHpDelta(''); if (!isHpEditing) updateCharacter(character.id, { hitPoints: n });
    };
          ...
          <CardHeader className="flex flex-col px-4 pt-2 pb-2 border-b bg-muted/10">
            ...
            {level > 0 && slots.max > 0 && (
              <div className="flex flex-wrap gap-1 mt-1">
                {Array.from({ length: slots.max }).map((_, i) => ( <Checkbox key={i} checked={slots.current > i} onCheckedChange={() => { const nextCurrent = slots.current === i + 1 ? i : i + 1; const n = { ...spellSlots, [level]: { ...slots, current: nextCurrent } }; setSpellSlots(n); updateCharacter(character.id, { spellSlots: n }); }} className="h-3 w-3" /> ))}
              </div>
            )}
          </CardHeader>
      ...
      <div className="space-y-8 pb-12">
        <div className={cn("bg-background/95 backdrop-blur-md -mt-4 md:-mt-6 -mx-4 px-4 py-3 border-b shadow-sm sticky top-0 z-30 space-y-3", !isCompactView && "hidden")}>
          <div className="grid grid-cols-6 gap-1">
            {['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma'].map(key => {
              ...
              return ( <div key={key} className="flex flex-col items-center justify-center bg-card border rounded py-1 min-w-0"><span className="text-[8px] font-bold text-muted-foreground uppercase truncate w-full text-center px-0.5">{key.slice(0,3)}</span><span className="text-xs font-black">{mod >= 0 ? `+${mod}` : mod}</span><span className="text-[8px] text-muted-foreground/60">{val}</span></div> );
            })}
          </div>
                    ...
                    <div className="pt-2 border-t space-y-4">
                      ...
                      {progressionData.isMulticlass && (
                        <div className="space-y-2">
                          ...
                          {isProgressionEditing && ( <Button variant="outline" size="sm" className="w-full h-7 text-[10px]" onClick={() => { const used = [progressionData.characterClass, ...progressionData.multiclasses.map(m => m.class)]; const avail = DND_CLASSES.filter(c => !used.includes(c)); if (avail.length > 0) setProgressionData({...progressionData, multiclasses: [...progressionData.multiclasses, { class: avail[0], level: 1 }]}); }}><Plus className="h-3 w-3 mr-1" /> Add Class</Button> )}
                        </div>
                      )}
                    </div>
        ...
        <div className={cn("grid grid-cols-1 md:grid-cols-12 gap-6 items-start", isCompactView && activeCompactSection !== 'stats-section' && "hidden")}>
          <div className="md:col-span-2 space-y-4">
            <Card><CardHeader className="flex flex-row items-center justify-between px-4 pt-2 pb-2"><CardTitle className="text-[10px] font-bold uppercase text-muted-foreground tracking-tight">{t('characteristics')}</CardTitle>{(showEditButtons || isStatsEditing) && <EditSaveButton editing={isStatsEditing} onEdit={() => setIsStatsEditing(true)} onSave={handleSaveStats} />}</CardHeader>
            <CardContent className="space-y-3 p-4 pt-0">{['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma'].map(key => ( <StatBox key={key} label={key} value={stats[key as keyof typeof stats]} editing={isStatsEditing} onChange={e => setStats({...stats, [key]: parseInt(e.target.value) || 1})} isCompactView={isCompactView} notes={character.statNotes?.[key as keyof typeof stats]} onNoteChange={v => updateCharacter(character.id, { statNotes: {...(character.statNotes || {}), [key]: v} })} hideNotes={hideNotes} /> ))}</CardContent></Card>
            
            <Card><CardHeader className="flex flex-row items-center justify-between px-4 pt-2 pb-2"><CardTitle className="text-[10px] font-bold uppercase text-muted-foreground tracking-tight">{t('otherProficienciesAndLanguages')}</CardTitle>{(showEditButtons || isOtherProficienciesEditing) && <EditSaveButton editing={isOtherProficienciesEditing} onEdit={() => setIsOtherProficienciesEditing(true)} onSave={handleSaveOtherProf} />}</CardHeader>
            <CardContent className="p-4 pt-0 space-y-2">
              {otherProficienciesAndLanguages.map((it, i) => (
                <div key={i} className="text-[10px] p-1.5 rounded bg-muted/10 flex justify-between">
                  {isOtherProficienciesEditing ? ( <Input value={it} onChange={e => { const n = [...otherProficienciesAndLanguages]; n[i] = e.target.value; setOtherProficienciesAndLanguages(n); }} className="h-6 text-[10px] flex-1 mr-2" /> ) : ( <span className="break-words font-medium">&bull; {it}</span> )}
              ...
              {isOtherProficienciesEditing && ( <div className="flex gap-2 pt-2 border-t"><Input placeholder="New..." value={newProfItem} onChange={e => setNewProfItem(e.target.value)} className="h-7 text-[10px]" /><Button size="sm" className="h-7" onClick={() => { if (newProfItem.trim()) { setOtherProficienciesAndLanguages([...otherProficienciesAndLanguages, newProfItem.trim()]); setNewProfItem(''); } }}><Plus className="h-3 w-3" /></Button></div> )}
            </CardContent></Card>
          </div>
          <div className="md:col-span-3 space-y-4">
            <Card id="saving-throws-card"><CardHeader className="flex flex-row items-center justify-between px-4 pt-2 pb-2"><CardTitle className="text-[10px] font-bold uppercase text-muted-foreground tracking-tight">{t('savingThrows')}</CardTitle>{(showEditButtons || isSavesEditing) && <EditSaveButton editing={isSavesEditing} onEdit={() => setIsSavesEditing(true)} onSave={handleSaveSaves} />}</CardHeader>
            <CardContent className="p-4 pt-0 space-y-1">{calculatedSavingThrows.map((st, i) => ( <div key={st.name} className="flex items-center gap-2 py-1 border-b last:border-0 border-muted"><Checkbox checked={st.proficient} disabled={!isSavesEditing} onCheckedChange={v => setSavingThrows(savingThrows.map((s, idx) => idx === i ? {...s, proficient: !!v} : s))} /><span className="font-bold text-sm w-8">{st.value >= 0 ? '+' : ''}{st.value}</span><span className="text-[10px] font-bold uppercase flex-1">{st.name}</span></div> ))}</CardContent></Card>
            <Card id="skills-card"><CardHeader className="flex flex-row items-center justify-between px-4 pt-2 pb-2"><CardTitle className="text-[10px] font-bold uppercase text-muted-foreground tracking-tight">{t('skills')}</CardTitle>{(showEditButtons || isSkillsEditing) && <EditSaveButton editing={isSkillsEditing} onEdit={() => setIsSkillsEditing(true)} onSave={handleSaveSkills} />}</CardHeader>
            <CardContent className="p-4 pt-0 space-y-1">{calculatedSkills.map((sk, i) => ( <div key={sk.name} className="flex items-center gap-2 py-1 border-b last:border-0 border-muted"><Checkbox checked={sk.proficient} disabled={!isSkillsEditing} onCheckedChange={v => setSkills(skills.map((s, idx) => idx === i ? {...s, proficient: !!v} : s))} /><div className="w-8 text-center">{isSkillsEditing ? ( <Input type="number" value={sk.value} onChange={e => { const n = [...skills]; n[i] = {...sk, value: parseInt(e.target.value) || 0}; setSkills(n); }} className="h-6 w-8 text-[10px] p-0 text-center font-bold" /> ) : ( <span className="font-black text-sm">{sk.value >= 0 ? '+' : ''}{sk.value}</span> )}</div><span className="text-xs font-semibold flex-1">{sk.label}</span></div> ))}</CardContent></Card>
          </div>
          <div className="md:col-span-4 space-y-6">
            <Card id="combat-stats-card"><CardHeader className="flex flex-row items-center justify-between px-4 pt-2 pb-2"><CardTitle className="text-[10px] font-bold uppercase text-muted-foreground tracking-tight">{t('combatStats')}</CardTitle>{(showEditButtons || isCombatStatsEditing) && <EditSaveButton editing={isCombatStatsEditing} onEdit={() => setIsCombatStatsEditing(true)} onSave={handleSaveCombatStats} />}</CardHeader>
            <CardContent className="grid grid-cols-3 gap-4 p-4 pt-0 text-center"><div className="p-2 rounded-lg bg-background border"><Label className="text-[10px] text-muted-foreground uppercase font-bold">{t('armorClass')}</Label>{isCombatStatsEditing ? ( <Input type="number" value={combatStats.armorClass} onChange={e => setCombatStats({...combatStats, armorClass: parseInt(e.target.value) || 10})} className="h-9 w-full text-center" /> ) : ( <div className="text-sm font-bold">{combatStats.armorClass}</div> )}</div><div className="p-2 rounded-lg bg-background border"><Label className="text-[10px] text-muted-foreground uppercase font-bold">{t('initiative')}</Label><div className="text-sm font-bold">+{dexMod}</div></div><div className="p-2 rounded-lg bg-background border"><Label className="text-[10px] text-muted-foreground uppercase font-bold">{t('speed')}</Label>{isCombatStatsEditing ? <Input type="number" value={combatStats.speed} onChange={e => setCombatStats({...combatStats, speed: parseInt(e.target.value) || 0})} className="h-9 w-full text-center" /> : <div className="text-sm font-bold">{combatStats.speed}ft</div>}</div></CardContent></Card>
            <Card id="hit-points-card"><CardHeader className="flex flex-row items-center justify-between px-4 pt-2 pb-2"><CardTitle className="text-[10px] font-bold uppercase text-muted-foreground tracking-tight">Health</CardTitle>{(showEditButtons || isHpEditing) && <EditSaveButton editing={isHpEditing} onEdit={() => setIsHpEditing(true)} onSave={handleSaveHp} />}</CardHeader>
            <CardContent className="p-4 pt-0 space-y-4"><div className="grid grid-cols-2 gap-4"><div className="space-y-4"><div className="p-2 border rounded text-center"><Label className="text-[10px] uppercase font-bold">Max</Label>{isHpEditing ? ( <Input type="number" value={combatStats.hitPoints.max} onChange={e => setCombatStats({...combatStats, hitPoints: {...combatStats.hitPoints, max: parseInt(e.target.value) || 0}})} className="h-8 text-center" /> ) : ( <div className="text-base font-bold">{combatStats.hitPoints.max}</div> )}</div><div className="p-2 border rounded text-center"><Label className="text-[10px] uppercase font-bold">Current</Label>{isHpEditing ? ( <Input type="number" value={combatStats.hitPoints.current} onChange={e => setCombatStats({...combatStats, hitPoints: {...combatStats.hitPoints, current: parseInt(e.target.value) || 0}})} className="h-8 text-center" /> ) : ( <div className="text-base font-bold text-primary">{combatStats.hitPoints.current}</div> )}</div></div><div className="space-y-4"><div className="p-2 border rounded text-center"><Label className="text-[10px] uppercase font-bold">Temp</Label>{isHpEditing ? ( <Input type="number" value={combatStats.temporaryHitPoints} onChange={e => setCombatStats({...combatStats, temporaryHitPoints: parseInt(e.target.value) || 0})} className="h-8 text-center" /> ) : ( <div className="text-base font-bold">{combatStats.temporaryHitPoints || 0}</div> )}</div><div className="flex flex-col gap-2"><Input type="number" placeholder="+/-" value={hpDelta} onChange={e => setHpDelta(e.target.value)} className="h-8 text-center" /><div className="grid grid-cols-2 gap-2"><Button size="sm" variant="outline" onClick={() => handleHpMath('sub')}>Sub</Button><Button size="sm" variant="outline" onClick={() => handleHpMath('rec')}>Heal</Button></div></div></div></div></CardContent></Card>
            <Card id="attacks-box"><CardHeader className="flex flex-row items-center justify-between px-4 pt-2 pb-2"><CardTitle className="text-[10px] font-bold uppercase text-muted-foreground tracking-tight">{t('attacksAndSpellcasting')}</CardTitle>{(showEditButtons || isAttacksEditing) && <EditSaveButton editing={isAttacksEditing} onEdit={() => setIsAttacksEditing(true)} onSave={handleSaveAttacks} />}</CardHeader>
            <CardContent className="p-4 pt-0 space-y-3">{attacks.map(atk => ( <div key={atk.id} className="flex items-center justify-between p-2 bg-muted/10 rounded border border-muted/20">{isAttacksEditing ? ( <div className="flex flex-col gap-1 flex-1 mr-2"><Input value={atk.name} onChange={e => setAttacks(attacks.map(a => a.id === atk.id ? {...a, name: e.target.value} : a))} className="h-7 text-xs" /><div className="flex gap-1"><Input value={atk.atkBonus} onChange={e => setAttacks(attacks.map(a => a.id === atk.id ? {...a, atkBonus: e.target.value} : a))} className="h-7 text-[10px] w-12" placeholder="Bonus" /><Input value={atk.damageType} onChange={e => setAttacks(attacks.map(a => a.id === atk.id ? {...a, damageType: e.target.value} : a))} className="h-7 text-[10px] flex-1" placeholder="Damage" /></div></div> ) : ( <div className="flex flex-col"><span className="text-xs font-black">{atk.name}</span><span className="text-xs text-muted-foreground">{atk.damageType}</span></div> )}<div className="flex items-center gap-2">{!isAttacksEditing && <span className="text-sm font-black text-primary">{atk.atkBonus}</span>}{isAttacksEditing && <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => setAttacks(attacks.filter(a => a.id !== atk.id))}><Trash2 className="h-3 w-3" /></Button>}</div></div> ))}{isAttacksEditing && <Button variant="outline" size="sm" className="w-full h-8 text-[10px]" onClick={() => setAttacks([...attacks, { id: `atk-${Date.now()}`, name: 'New Attack', atkBonus: '+0', damageType: '1d6', notes: '' }])}><Plus className="mr-1 h-3 w-3" /> Add Attack</Button>}</CardContent></Card>
          </div>
          <div className="md:col-span-3 space-y-6">
            ...
            <CardContent className="p-4 pt-0 space-y-2">{narrativeData.personalityTraits.map((it, i) => ( <div key={i} className="text-[10px] p-1.5 rounded bg-muted/10 flex justify-between">{isTraitEditing ? ( <Input value={it} onChange={e => { const n = [...narrativeData.personalityTraits]; n[i] = e.target.value; setNarrativeData({...narrativeData, personalityTraits: n}); }} className="h-6 text-[10px] flex-1 mr-2" /> ) : ( <span className="break-words font-medium">&bull; {it}</span> )}{isTraitEditing && ( <Button variant="ghost" size="icon" className="h-5 w-5 text-destructive" onClick={() => setNarrativeData({...narrativeData, personalityTraits: narrativeData.personalityTraits.filter((_, idx) => idx !== i)}) }><Trash2 className="h-3 w-3" /></Button> )}</div> ))}{isTraitEditing && ( <div className="flex gap-2 pt-2 border-t"><Input placeholder="New..." value={newTraitItem} onChange={e => setNewTraitItem(e.target.value)} className="h-7 text-[10px]" /><Button size="sm" className="h-7" onClick={() => { if (newTraitItem.trim()) { setNarrativeData({...narrativeData, personalityTraits: [...narrativeData.personalityTraits, newTraitItem.trim()]}); setNewTraitItem(''); } }}><Plus className="h-3 w-3" /></Button></div> )}</CardContent></Card>
            ...
            <CardContent className="p-4 pt-0 space-y-2">{narrativeData.ideals.map((it, i) => ( <div key={i} className="text-[10px] p-1.5 rounded bg-muted/10 flex justify-between">{isIdealEditing ? ( <Input value={it} onChange={e => { const n = [...narrativeData.ideals]; n[i] = e.target.value; setNarrativeData({...narrativeData, ideals: n}); }} className="h-6 text-[10px] flex-1 mr-2" /> ) : ( <span className="break-words font-medium">&bull; {it}</span> )}{isIdealEditing && ( <Button variant="ghost" size="icon" className="h-5 w-5 text-destructive" onClick={() => setNarrativeData({...narrativeData, ideals: narrativeData.ideals.filter((_, idx) => idx !== i)}) }><Trash2 className="h-3 w-3" /></Button> )}</div> ))}{isIdealEditing && ( <div className="flex gap-2 pt-2 border-t"><Input placeholder="New..." value={newIdealItem} onChange={e => setNewIdealItem(e.target.value)} className="h-7 text-[10px]" /><Button size="sm" className="h-7" onClick={() => { if (newIdealItem.trim()) { setNarrativeData({...narrativeData, ideals: [...narrativeData.ideals, newIdealItem.trim()]}); setNewIdealItem(''); } }}><Plus className="h-3 w-3" /></Button></div> )}</CardContent></Card>
            ...
            <CardContent className="p-4 pt-0 space-y-2">{narrativeData.bonds.map((it, i) => ( <div key={i} className="text-[10px] p-1.5 rounded bg-muted/10 flex justify-between">{isBondEditing ? ( <Input value={it} onChange={e => { const n = [...narrativeData.bonds]; n[i] = e.target.value; setNarrativeData({...narrativeData, bonds: n}); }} className="h-6 text-[10px] flex-1 mr-2" /> ) : ( <span className="break-words font-medium">&bull; {it}</span> )}{isBondEditing && ( <Button variant="ghost" size="icon" className="h-5 w-5 text-destructive" onClick={() => setNarrativeData({...narrativeData, bonds: narrativeData.bonds.filter((_, idx) => idx !== i)}) }><Trash2 className="h-3 w-3" /></Button> )}</div> ))}{isBondEditing && ( <div className="flex gap-2 pt-2 border-t"><Input placeholder="New..." value={newBondItem} onChange={e => setNewBondItem(e.target.value)} className="h-7 text-[10px]" /><Button size="sm" className="h-7" onClick={() => { if (newBondItem.trim()) { setNarrativeData({...narrativeData, bonds: [...narrativeData.bonds, newBondItem.trim()]}); setNewBondItem(''); } }}><Plus className="h-3 w-3" /></Button></div> )}</CardContent></Card>
            ...
            <CardContent className="p-4 pt-0 space-y-2">{narrativeData.flaws.map((it, i) => ( <div key={i} className="text-[10px] p-1.5 rounded bg-muted/10 flex justify-between">{isFlawEditing ? ( <Input value={it} onChange={e => { const n = [...narrativeData.flaws]; n[i] = e.target.value; setNarrativeData({...narrativeData, flaws: n}); }} className="h-6 text-[10px] flex-1 mr-2" /> ) : ( <span className="break-words font-medium">&bull; {it}</span> )}{isFlawEditing && ( <Button variant="ghost" size="icon" className="h-5 w-5 text-destructive" onClick={() => setNarrativeData({...narrativeData, flaws: narrativeData.flaws.filter((_, idx) => idx !== i)}) }><Trash2 className="h-3 w-3" /></Button> )}</div> ))}{isFlawEditing && ( <div className="flex gap-2 pt-2 border-t"><Input placeholder="New..." value={newFlawItem} onChange={e => setNewFlawItem(e.target.value)} className="h-7 text-[10px]" /><Button size="sm" className="h-7" onClick={() => { if (newFlawItem.trim()) { setNarrativeData({...narrativeData, flaws: [...narrativeData.flaws, newFlawItem.trim()]}); setNewFlawItem(''); } }}><Plus className="h-3 w-3" /></Button></div> )}</CardContent></Card>
            ...
            <CardContent className="p-4 pt-0 space-y-2">{narrativeData.featuresAndTraits.map((it, i) => ( <div key={i} className="text-[10px] p-1.5 rounded bg-muted/10 flex justify-between">{isFeaturesEditing ? ( <Input value={it} onChange={e => { const n = [...narrativeData.featuresAndTraits]; n[i] = e.target.value; setNarrativeData({...narrativeData, featuresAndTraits: n}); }} className="h-6 text-[10px] flex-1 mr-2" /> ) : ( <span className="break-words font-medium">&bull; {it}</span> )}{isFeaturesEditing && ( <Button variant="ghost" size="icon" className="h-5 w-5 text-destructive" onClick={() => setNarrativeData({...narrativeData, featuresAndTraits: narrativeData.featuresAndTraits.filter((_, idx) => idx !== i)}) }><Trash2 className="h-3 w-3" /></Button> )}</div> ))}{isFeaturesEditing && ( <div className="flex gap-2 pt-2 border-t"><Input placeholder="New..." value={newFeatureItem} onChange={e => setNewFeatureItem(e.target.value)} className="h-7 text-[10px]" /><Button size="sm" className="h-7" onClick={() => { if (newFeatureItem.trim()) { setNarrativeData({...narrativeData, featuresAndTraits: [...narrativeData.featuresAndTraits, newFeatureItem.trim()]}); setNewFeatureItem(''); } }}><Plus className="h-4 w-4" /></Button></div> )}</CardContent></Card>

            <Card id="attunement-card"><CardHeader className="flex flex-row items-center justify-between px-4 pt-2 pb-2"><CardTitle className="text-[10px] font-bold uppercase text-muted-foreground tracking-tight">{t('attunement')}</CardTitle>{(showEditButtons || isAttunementEditing) && <EditSaveButton editing={isAttunementEditing} onEdit={() => setIsAttunementEditing(true)} onSave={handleSaveAttunement} />}</CardHeader>
            <CardContent className="p-4 pt-0 space-y-2">
            ...
            <Card id="inventory-box"><CardHeader className="px-4 pt-2 pb-2"><CardTitle className="text-[10px] font-bold uppercase text-muted-foreground tracking-tight">{t('inventory')}</CardTitle></CardHeader>
            <CardContent className="p-4 pt-0 space-y-6"><div className="border rounded-lg p-3 bg-muted/30"><div className="flex items-center justify-between mb-3 border-b pb-1"><Label className="text-[10px] uppercase font-bold">{t('money')}</Label>{(showEditButtons || isMoneyEditing) && <EditSaveButton editing={isMoneyEditing} onEdit={() => setIsMoneyEditing(true)} onSave={handleSaveMoney} />}</div><div className="grid grid-cols-5 gap-2">{['cp', 'sp', 'ep', 'gp', 'pp'].map(c => ( <div key={c} className="flex flex-col items-center"><span className="text-[8px] font-bold uppercase">{c}</span>{isMoneyEditing ? ( <Input type="number" value={currency[c as keyof typeof currency]} onChange={e => setCurrency({...currency, [c]: parseInt(e.target.value) || 0})} className="h-6 p-0 text-center text-[10px]" /> ) : ( <div className="text-sm font-bold">{currency[c as keyof typeof currency]}</div> )}</div> ))}</div></div>
            <div className="space-y-2"><div className="flex items-center justify-between"><Label className="text-[10px] uppercase font-bold">{t('items')}</Label>{(showEditButtons || isItemsEditing) && <EditSaveButton editing={isItemsEditing} onEdit={() => setIsItemsEditing(true)} onSave={handleSaveItems} />}</div><ul className="space-y-1 text-xs">{equipment.map(it => ( <li key={it.id} className="flex items-center justify-between group">{isItemsEditing ? ( <div className="flex items-center gap-2 flex-1"><Checkbox checked={it.status === 'lost'} onCheckedChange={v => setEquipment(equipment.map(i => i.id === it.id ? {...i, status: v ? 'lost' : 'default'} : i))} /><Input value={it.name} onChange={e => setEquipment(equipment.map(i => i.id === it.id ? {...i, name: e.target.value} : i))} className="h-7 text-xs flex-1" /><Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => setEquipment(equipment.filter(i => i.id !== it.id))}><Trash2 className="h-3 w-3" /></Button></div> ) : ( <span className={cn(it.status === 'lost' && "line-through")}>&bull; {it.name}</span> )}</li> ))}{isItemsEditing && ( <div className="flex gap-2 pt-2 border-t"><Input placeholder="New..." value={newEquipmentItem} onChange={e => setNewEquipmentItem(e.target.value)} className="h-8 text-xs" /><Button size="sm" onClick={() => { if (newEquipmentItem.trim()) { setEquipment([...equipment, { id: `eq-${Date.now()}`, name: newEquipmentItem.trim(), status: 'default' }]); setNewEquipmentItem(''); } }}><Plus className="h-4 w-4" /></Button></div> )}</ul></div></CardContent></Card>
          </div>
        </div>
        ...
        <div className={cn("space-y-6", isCompactView && activeCompactSection !== 'spells-section' && "hidden")}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">{renderSpellBox(0, t('cantrips'))}{[1, 2, 3, 4, 5, 6, 7, 8, 9].map(lvl => renderSpellBox(lvl, `${t('level')} ${lvl}`))}</div>
        </div>
        ...
        <div className={cn("space-y-6", isCompactView && activeCompactSection !== 'companion-section' && "hidden")}>
          ...
          {companions.map(comp => (
            <Card key={comp.id} className="p-4 border-2"><CardHeader className="flex flex-row items-center justify-between p-0 mb-4"><CardTitle className="text-lg font-bold">{comp.name}</CardTitle><Button variant="ghost" size="icon" className="text-destructive" onClick={() => { const n = companions.filter(c => c.id !== comp.id); setCompanions(n); updateCharacter(character.id, { companions: n }); }}><Trash2 className="h-4 w-4" /></Button></CardHeader>
            <CardContent className="p-0 space-y-4">
              <div className="grid grid-cols-2 gap-4"><DetailField label="Type" value={comp.type} editing={showEditButtons} onChange={v => { const n = companions.map(c => c.id === comp.id ? { ...c, type: v } : c); setCompanions(n); updateCharacter(character.id, { companions: n }); }} /><DetailField label="Size" value={comp.size} editing={showEditButtons} onChange={v => { const n = companions.map(c => c.id === comp.id ? { ...c, size: v } : c); setCompanions(n); updateCharacter(character.id, { companions: n }); }} /></div>
              <div className="p-2 border rounded bg-muted/5 text-center"><Label className="text-[10px] uppercase font-bold">HP</Label><div className="flex justify-center items-center gap-2">{showEditButtons ? ( <><Input type="number" value={comp.hitPoints.current} onChange={e => { const n = companions.map(c => c.id === comp.id ? { ...c, hitPoints: {...c.hitPoints, current: parseInt(e.target.value) || 0} } : c); setCompanions(n); updateCharacter(character.id, { companions: n }); }} className="h-7 w-12 text-center" /><span>/</span><Input type="number" value={comp.hitPoints.max} onChange={e => { const n = companions.map(c => c.id === comp.id ? { ...c, hitPoints: {...c.hitPoints, max: parseInt(e.target.value) || 0} } : c); setCompanions(n); updateCharacter(character.id, { companions: n }); }} className="h-7 w-12 text-center" /></> ) : ( <span className="text-lg font-bold">{comp.hitPoints.current} / {comp.hitPoints.max}</span> )}</div></div>
            </CardContent></Card>
          ))}
        </div>
      </div>
