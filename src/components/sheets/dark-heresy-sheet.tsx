...
import { getPointsObject, getSimplePointsObject } from '@/lib/character-utils';
import { EditSaveButton } from '@/components/ui/edit-save-button';
import { ArmorDisplay, ArmorSection } from './sections/dark-heresy-armor';
import { SkillsSection } from './sections/dark-heresy-skills';
import { WeaponsSection } from './sections/dark-heresy-weapons';
import { InventorySection } from './sections/dark-heresy-inventory';
import { TalentsSection } from './sections/dark-heresy-talents';
import { useDebouncedCallback } from '@/hooks/use-debounced-callback';
import { getAdvancedPathThreshold, canChooseAlternateRank } from '@/lib/dark-heresy-career';
import { RanksByCareer, AdvancedPathsByCareer, AdvancedPath, calculateRank } from '@/lib/dark-heresy-ranks';
import { calculateStatUpgrades } from '@/lib/dark-heresy-stat-upgrades';
...
export const DarkHeresySheet = React.forwardRef<any, { character: DarkHeresyCharacter, isCompactView: boolean, activeCompactSection: string }>(
  ({ character, isCompactView, activeCompactSection }, ref) => {
    ...
    const handleSkillChange = (skillId: string, field: keyof Skill, value: any) => { setEditableSkills(prev => prev.map(s => s.id === skillId ? { ...s, [field]: value } : s));};
    ...
    const advancedPathThreshold = getAdvancedPathThreshold(character.careerPath);
    ...
    const debouncedHandlePointChange = useDebouncedCallback(handlePointChange, 300);
    ...
    const handleUpgradeChange = (statKey: keyof typeof defaultStatUpgrades, index: number, isChecked: boolean) => {
        if (isCharacteristicsEditing) {
            setEditableStatUpgrades(prev => calculateStatUpgrades(prev, statKey, index, isChecked));
        } else {
            const next = calculateStatUpgrades(character.statUpgrades ?? defaultStatUpgrades, statKey, index, isChecked);
    ...
    const canChooseAltRank = canChooseAlternateRank(character.careerPath, editableTotalExpSpent);
                            ...
                            <MetricBox title={t('wounds')} notes={getPointsObject(character.wounds).notes} onNoteChange={(val) => updateCharacter(character.id, { wounds: { ...getPointsObject(character.wounds), notes: val } })} editing={isPointsEditing} onEdit={() => setIsPointsEditing(true)} onSave={handleSavePoints} isCompactView={isCompactView} hideNotes={hideNotes} showEditButtons={showEditButtons}>
                              <div className="flex items-center justify-center gap-1">
                                {isPointsEditing ? (
                                  ...
                                  <div className="flex items-center justify-center gap-1">
                                    <Button size="icon" variant="outline" className="h-5 w-5" onClick={() => debouncedHandlePointChange('wounds', -1)}><Minus className="h-3 w-3" /></Button>
                                    <span className="font-bold text-sm min-w-[1.2rem] text-center">{getPointsObject(character.wounds).current}</span>
                                    <Button size="icon" variant="outline" className="h-5 w-5" onClick={() => debouncedHandlePointChange('wounds', 1)}><Plus className="h-3 w-3" /></Button>
                            ...
                            <MetricBox title={t('fate')} notes={getPointsObject(character.fatePoints).notes} onNoteChange={(val) => updateCharacter(character.id, { fatePoints: { ...getPointsObject(character.fatePoints), notes: val } })} editing={isPointsEditing} onEdit={() => setIsPointsEditing(true)} onSave={handleSavePoints} isCompactView={isCompactView} hideNotes={hideNotes} showEditButtons={showEditButtons}>
                              <div className="flex items-center justify-center gap-1">
                                {isPointsEditing ? (
                                  ...
                                  <div className="flex items-center justify-center gap-1">
                                    <Button size="icon" variant="outline" className="h-5 w-5" onClick={() => debouncedHandlePointChange('fatePoints', -1)}><Minus className="h-3 w-3" /></Button>
                                    <span className="font-bold text-sm min-w-[1.2rem] text-center">{getPointsObject(character.fatePoints).current}</span>
                                    <Button size="icon" variant="outline" className="h-5 w-5" onClick={() => debouncedHandlePointChange('fatePoints', 1)}><Plus className="h-3 w-3" /></Button>
                                    <span className="text-[10px] text-muted-foreground mx-0.5">/</span>
                            ...
                            <MetricBox title={t('insanity')} notes={getSimplePointsObject(character.insanityPoints).notes} onNoteChange={(val) => updateCharacter(character.id, { insanityPoints: { ...getSimplePointsObject(character.insanityPoints), notes: val } })} editing={isPointsEditing} onEdit={() => setIsPointsEditing(true)} onSave={handleSavePoints} isCompactView={isCompactView} hideNotes={hideNotes} showEditButtons={showEditButtons}>
                              <div className="flex items-center justify-center gap-1">
                                {isPointsEditing ? ( 
                                  ...
                                  <div className="flex items-center justify-center gap-1">
                                    <Button size="icon" variant="outline" className="h-5 w-5" onClick={() => debouncedHandlePointChange('insanityPoints', -1)}><Minus className="h-3 w-3" /></Button>
                                    <span className="font-bold text-sm min-w-[1.2rem] text-center">{getSimplePointsObject(character.insanityPoints).total}</span>
                                    <Button size="icon" variant="outline" className="h-5 w-5" onClick={() => debouncedHandlePointChange('insanityPoints', 1)}><Plus className="h-3 w-3" /></Button>
                                  </div>
                                )}
                              </div>
                            </MetricBox>
                            <MetricBox title={t('corruption')} notes={getSimplePointsObject(character.corruptionPoints).notes} onNoteChange={(val) => updateCharacter(character.id, { corruptionPoints: { ...getSimplePointsObject(character.corruptionPoints), notes: val } })} editing={isPointsEditing} onEdit={() => setIsPointsEditing(true)} onSave={handleSavePoints} isCompactView={isCompactView} hideNotes={hideNotes} showEditButtons={showEditButtons}>
                              <div className="flex items-center justify-center gap-1">
                                {isPointsEditing ? ( 
                                  ...
                                  <div className="flex items-center justify-center gap-1">
                                    <Button size="icon" variant="outline" className="h-5 w-5" onClick={() => debouncedHandlePointChange('corruptionPoints', -1)}><Minus className="h-3 w-3" /></Button>
                                    <span className="font-bold text-sm min-w-[1.2rem] text-center">{getSimplePointsObject(character.corruptionPoints).total}</span>
                                    <Button size="icon" variant="outline" className="h-5 w-5" onClick={() => debouncedHandlePointChange('corruptionPoints', 1)}><Plus className="h-3 w-3" /></Button>
                                  </div>
                                )}
                              </div>
                            </MetricBox>
                            ...
                            <CardContent className="pt-0 space-y-8">
                                                ...
                                                <Table>
                                                    ...
                                                    <TableBody>
                                                        <SkillsSection
                                                            ...
                                                            isSkillsExpanded={!isSkillsPanelCollapsed}
                                                            setIsSkillsExpanded={setIsSkillsPanelCollapsed}
                                ...
                                <div id="talents-section" className={cn(isCompactView && activeCompactSection !== 'talents-section' && 'hidden')}>
                                    <TalentsSection
                                        character={character}
                                        isCompactView={isCompactView}
                                        isTalentsEditing={isTalentsEditing}
                                        setIsTalentsEditing={setIsTalentsEditing}
                                        handleSaveTalents={handleSaveTalents}
                                        editableTalents={editableTalents}
                                        setEditableTalents={setEditableTalents}
                                            ...
                                            <div className="p-6 space-y-6">
                                                <div>
                                                    <ArmorSection
                                                        ...
                                                        isArmorEditing={isArmorEditing}
                                                        setIsArmorEditing={setIsArmorEditing}
                                                        handleSaveArmor={handleSaveArmor}
                                                        handleArmorChange={handleArmorChange}
                                                        isArmorExpanded={isArmorExpanded}
                                                        setIsArmorExpanded={setIsArmorExpanded}
                                                        showEditButtons={showEditButtons}
                                                    />
                                                </div>
                                                <WeaponsSection
                                                    ...
                                                    isCompactView={isCompactView}
                                                    currentEquipment={currentEquipment}
                                                    isMeleeWeaponsEditing={isMeleeWeaponsEditing}
                                                    setIsMeleeWeaponsEditing={setIsMeleeWeaponsEditing}
                                                    handleSaveMeleeWeapons={handleSaveMeleeWeapons}
                                                    handleMeleeWeaponChange={handleMeleeWeaponChange}
                                                    addMeleeWeapon={addMeleeWeapon}
                                                    removeMeleeWeapon={removeMeleeWeapon}
                                                    isRangedWeaponsEditing={isRangedWeaponsEditing}
                                                    setIsRangedWeaponsEditing={setIsRangedWeaponsEditing}
                                                    handleSaveRangedWeapons={handleSaveRangedWeapons}
                                                    handleRangedWeaponChange={handleRangedWeaponChange}
                                                    addRangedWeapon={addRangedWeapon}
                                                    removeRangedWeapon={removeRangedWeapon}
                                                    showEditButtons={showEditButtons}
                                                    hideNotes={hideNotes}
                                                    updateCharacter={updateCharacter}
                                                />
                                    ...
                                    <Collapsible id="inventory-section" open={isInventorySectionOpen} onOpenChange={setIsInventorySectionOpen} className="border rounded-lg overflow-hidden">
                                        ...
                                        <CollapsibleContent>
                                            <InventorySection
                                                ...
                                                editableWealth={editableWealth}
                                                isWealthEditing={isWealthEditing}
                                                setIsWealthEditing={setIsWealthEditing}
                                                handleSaveWealth={handleSaveWealth}
                                                setEditableWealth={setEditableWealth}
                                                isInventoryEditing={isInventoryEditing}
                                                setIsInventoryEditing={setIsInventoryEditing}
                                                handleSaveInventory={handleSaveInventory}
                                                editableInventory={editableInventory}
                                                setEditableInventory={setEditableInventory}
